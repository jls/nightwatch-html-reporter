...
module.exports = function(opts){
  ...
  // Stores a map of package names to package
  // objects so we don't have to search the
  // packages list every time we add a suite.
  var _packageCache = {};
  this.addSuite = function(data, moduleName)
  {
    ...
    if(_packageCache[suite.package]) {
      package = _packageCache[suite.package];
    } else {
      package = new TestPackage(suite.package);
      _packageCache[suite.package] = package;
