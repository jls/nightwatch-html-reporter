...
nconf.argv({
  ...
  'f': {
    alias: 'browser-fail',
    describe:
      'If true, generated report will only open in browser on test failure.',
    default: false,
  },
  c: {
    alias: 'compact',
  ...
  'customTheme': {
    describe: 'Relative path to custom theme. When this is given, `themeName` will be ignored.'
  },
...
var opts = {
  ...
  openBrowserOnlyOnFailure: nconf.get('browser-fail') === true,
...
async.waterfall([
  ...
  if ((opts.openBrowser && !opts.openBrowserOnlyOnFailure) || (opts.openBrowserOnlyOnFailure && run.isFailure))
    open(reportFilename);
});
